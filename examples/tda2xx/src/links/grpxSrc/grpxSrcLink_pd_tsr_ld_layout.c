...
Int32 GrpxSrcLink_drawPdTsrLdLayout(GrpxSrcLink_Obj *pObj)
{
    ...
    startX = (pObj->info.queInfo[0].chInfo[0].width/2 - strlen(loadString)*fontProp.width)/2;
    if(startX<0)
        startX = 0;
    ...
    snprintf(loadString, GRPX_SRC_LINK_STR_SZ,"LANE DETECTION");
    ...
    Draw2D_drawString(pObj->draw2DHndl,
                      pObj->info.queInfo[0].chInfo[0].width/2 + startX,
                      pObj->info.queInfo[0].chInfo[0].height/4 - fontProp.height,
                      loadString,
                      &fontPrm);
